...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	conn := NewClientConn(ws, claims.RoomUUID, claims.UserUUID, h.pingEvery, h.pongWait, h.maxBytes, h.maxPerSec, h.sessions)
	...
	if err := h.sessions.Connect(ctx, conn, claims.RoomUUID, claims.UserUUID); err != nil {
		reason := "room_check"
		switch {
		case errors.Is(err, model.ErrRoomOnOtherInstance):
			reason = "affinity"
